...
import com.sunshine.ninjafruit.entities.Asteroid;
import com.sunshine.ninjafruit.entities.Bullet;
import com.sunshine.ninjafruit.entities.Explosion;
import com.sunshine.ninjafruit.tools.CollisionRect;
...
public class MainGameScreen implements Screen {
	...
	public MainGameScreen (NinjaFruitGame game) {
		this.game = game;
		y = 15;
		x = NinjaFruitGame.WIDTH / 2 - SHIP_WIDTH / 2;
	...
	public void render (float delta) {
		...
		if (asteroidSpawnTimer <= 0) {
			asteroidSpawnTimer = random.nextFloat() * (MAX_ASTEROID_SPAWN_TIME - MIN_ASTEROID_SPAWN_TIME) + MIN_ASTEROID_SPAWN_TIME;
			asteroids.add(new Asteroid(random.nextInt(NinjaFruitGame.WIDTH - Asteroid.WIDTH)));
		}
		...
		if (isRight()) {//Right
			...
			if (x + SHIP_WIDTH > NinjaFruitGame.WIDTH)
				x = NinjaFruitGame.WIDTH - SHIP_WIDTH;
		...
		scoreFont.draw(game.batch, scoreLayout, NinjaFruitGame.WIDTH / 2 - scoreLayout.width / 2, NinjaFruitGame.HEIGHT - scoreLayout.height - 10);
		...
		game.batch.draw(blank, 0, 0, NinjaFruitGame.WIDTH * health, 5);
		...
		if (showControls) {
			if (NinjaFruitGame.IS_MOBILE) {
				//Draw left
				game.batch.setColor(Color.RED);
				game.batch.draw(controls, 0, 0, NinjaFruitGame.WIDTH / 2, NinjaFruitGame.HEIGHT, 0, 0, NinjaFruitGame.WIDTH / 2, NinjaFruitGame.HEIGHT, false, false);
	
				//Draw right
				game.batch.setColor(Color.BLUE);
				game.batch.draw(controls, NinjaFruitGame.WIDTH / 2, 0, NinjaFruitGame.WIDTH / 2, NinjaFruitGame.HEIGHT, 0, 0, NinjaFruitGame.WIDTH / 2, NinjaFruitGame.HEIGHT, true, false);
				
				game.batch.setColor(Color.WHITE);
			} else {
				GlyphLayout instructionsLayout = new GlyphLayout(scoreFont, "Press Left/Right Arrowkeys to Shoot!", Color.WHITE, NinjaFruitGame.WIDTH - 50, Align.center, true);
				scoreFont.draw(game.batch, instructionsLayout, NinjaFruitGame.WIDTH / 2 - instructionsLayout.width / 2, 150);
			}
		}
	...
	private boolean isRight () {
		return Gdx.input.isKeyPressed(Keys.RIGHT) || (Gdx.input.isTouched() && game.cam.getInputInGameWorld().x >= NinjaFruitGame.WIDTH / 2);
	}
	
	private boolean isLeft () {
		return Gdx.input.isKeyPressed(Keys.LEFT) || (Gdx.input.isTouched() && game.cam.getInputInGameWorld().x < NinjaFruitGame.WIDTH / 2);
	}
	
	private boolean isJustRight () {
		return Gdx.input.isKeyJustPressed(Keys.RIGHT) || (Gdx.input.justTouched() && game.cam.getInputInGameWorld().x >= NinjaFruitGame.WIDTH / 2);
	}
	
	private boolean isJustLeft () {
		return Gdx.input.isKeyJustPressed(Keys.LEFT) || (Gdx.input.justTouched() && game.cam.getInputInGameWorld().x < NinjaFruitGame.WIDTH / 2);
	}
